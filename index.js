...
function CemberinAlani(radius, pi) {
  const area = pi * Math.pow(radius, 2); // radius * radius or radius ** 2
  return area;
}
...
sayilar.forEach((number) => {
  return number % 3 === 0 ? ucetambolunenler.push(number) : null;
});

// sayilar.forEach((number) =>
//   number % 3 === 0 ? ucetambolunenler.push(number) : null
// );
...
//3c çözümü:
ucebolunenlerintoplami = sayilar.reduce((acc, number) => {
  return number % 3 === 0 ? (acc += number) : acc;
}, 0);
...
for (let i = 0; i < sayilar.length; i++) {
  ...
  for (let k = 0; k < sayilar.length; k++) {
    if (sayilar[i] === sayilar[k]) {
      counter++;
      if (counter > 1) repeatNumbers[sayilar[i]] = counter;
    }
  }
}
...
for (const [key, value] of Object.entries(repeatNumbers)) {
